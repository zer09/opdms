...
import { GetResponse } from '../interface/response/get-response';
...
export class ServerAddressService {
  ...
  public getDefaultServer(): Promise<ServerKeyAddress> {
    return new Promise<ServerKeyAddress>(resolve => {
      this._localStorage.get(this._defaultAddress)
        .then(adrName => {
          this._localStorage.get(this._keyPrefix + adrName).then(adr => {
            resolve({
              key: adrName,
              address: adr
            });
  ...
  public getDefaultServerAPI(): Promise<string> {
    return this.getDefaultServer().then(ska => ska.address + '/api');
  }

  public getDefaultServerStore(): Promise<string> {
    return this.getDefaultServer().then(ska => ska.address + '/store');
  }
}
